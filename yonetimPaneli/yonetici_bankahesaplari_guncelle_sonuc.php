<?php
    ...
    if(isset($_SESSION["kullanici_Yonetici"])){
        ...
        if(($gelen_id != "") and ($gelen_bankaAdi != "") and ($gelen_bankaSubeAdi != "") and ($gelen_bankaSubeKodu != "") and ($gelen_bankaSehir != "") and ($gelen_bankaUlke != "") and ($gelen_bankaParaBirimi != "") and ($gelen_bankaHesapSahibi != "") and ($gelen_bankaHesapNumarasi != "") and ($gelen_bankaHesapIban != "")){
            $sorgu_bankaHesabiGuncelle = $veritaConn -> prepare("UPDATE banka_hesaplarimiz SET BankaAdi = ?, KonumSehir = ?, KonumUlke = ?, SubeAdi = ?, SubeKodu = ?, ParaBirimi = ?, HesapSahibi = ?, HesapNumarasi = ?, IbanNumarasi = ? WHERE id = ? LIMIT 1");
            $sorgu_bankaHesabiGuncelle -> execute([$gelen_bankaAdi, $gelen_bankaSehir, $gelen_bankaUlke, $gelen_bankaSubeAdi, $gelen_bankaSubeKodu, $gelen_bankaParaBirimi, $gelen_bankaHesapSahibi, $gelen_bankaHesapNumarasi, $gelen_bankaHesapIban, $gelen_id]);
            $guncellemeSayisi = $sorgu_bankaHesabiGuncelle -> rowCount();
            /*
                ...
                */
                ...
                if($is_logoYukle -> uploaded){
                    ...
                    if($is_logoYukle -> processed){
                        $is_logoYukle -> clean();
                        $sorgu_logoYoluGuncelle = $veritaConn -> prepare("UPDATE banka_hesaplarimiz SET BankaLogosu = ? WHERE id = ? LIMIT 1");
