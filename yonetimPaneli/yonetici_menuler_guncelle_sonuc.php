<?php
    if(isset($_SESSION["kullanici_Yonetici"])){
        ...
        if(($gelen_id != "") and ($gelen_menuAdi != "")){
            $sorgu_menuGuncelle = $veritaConn -> prepare("UPDATE menuler SET menu_ad = ? WHERE id = ? LIMIT 1");
            $sorgu_menuGuncelle -> execute([$gelen_menuAdi, $gelen_id]);
