<?php
    ...
    $sorgu_toplamKayitSayisi = $veritaConn -> prepare("SELECT * FROM urunler WHERE urun_tur = 'Takı Seti' AND urun_durum = '1'" . $menuKosulu . " " . $aramaKosulu . "");
    ...
    $sorgu_menulerinStokSayilariToplami = $veritaConn -> prepare("SELECT SUM(urun_sayi) AS toplamUrunStogu FROM menuler WHERE urun_tur = 'Takı Seti'");
                        ...
                        <table width="250" align="center" border="0" cellpadding="0" cellspacing="0">
                            ...
                            <tr height="30">
                                <td class="beaulinkforanchorbold">
                                    <a href="index.php?SO=56" style="text-decoration: none; <?php if($gelen_menuId == ""){ ?> color: #646464;<?php }else{ ?> <?php } ?> font-weight: bold;">&nbsp;Tüm Ürünler (<?php echo $menulerinStokSayilariToplami["toplamUrunStogu"]; ?>)</a></td>
                                </td>
                            </tr>
                                <?php
                                    $sorgu_menuler = $veritaConn -> prepare("SELECT * FROM menuler WHERE urun_tur = 'Takı Seti' ORDER BY menu_ad ASC");
                                    ...
                                    foreach($menuler as $menu){
                                        ...
                                        <tr height="30">
                                            <td class="beaulinkforanchorbold">
                                                <a href="index.php?SO=56&menu_id=<?php echo $menu["id"]; ?>" style="<?php if($gelen_menuId == $menu["id"]){ ?>color: #646464;<?php }else{ ?> <?php } ?>">&nbsp;<?php echo DonusumleriGeriDondur($menu["menu_ad"]); ?>(<?php echo DonusumleriGeriDondur($menu["urun_sayi"]); ?>)</a>
                                            </td>
                                        </tr>
                                        <?php
                                    }
                ...
                <tr>
                    <td>
                        <div class="aramaAlani">
                            <form action="<?php if($menuKosulu!=""){ ?>index.php?SO=56<?php echo "&menu_id=" . $gelen_menuId; }else{ ?> <?php } ?>" method="post">
                ...
                <tr>
                    <td>
                        <table width="795" align="center" border="0" cellpadding="0" cellspacing="0">
                            <tr>
                                <?php
                                    $sorgu_urunler = $veritaConn -> prepare("SELECT * FROM urunler WHERE urun_tur = 'Takı Seti' AND urun_durum = '1'" . $menuKosulu . " " . $aramaKosulu. " ORDER BY id DESC LIMIT $sf_sayfalamayaBaslanacakKayitSayisi, $sf_kayitSayisi"); //
                        ...
                        <div class="pagingContainer">
                            ...
                            <div class="pagingContainerTextAreaForNumbers">
                                <?php
                                    if($sf_totalPageCount > 1){
                                        if($sf_value > 1){
                                            echo "<span class='pagePassive'><a href='index.php?SO=56" . $sayfalamaKosulu . "&SF=1'><<</a></span>";
                                            
                                            $sf_decreasedValue = $sf_value - 1;
                                            echo "<span class='pagePassive'><a href='index.php?SO=56" . $sayfalamaKosulu . "&SF=" . $sf_decreasedValue . "'><</a></span>";
                                        }
                                        for($sf_indexValue = ($sf_value - $sf_butonSayisi); $sf_indexValue <= ($sf_value + $sf_butonSayisi); $sf_indexValue++){
                                            if(($sf_indexValue > 0) and ($sf_indexValue <= $sf_totalPageCount)){
                                                if($sf_value == $sf_indexValue){
                                                    ...
                                                    echo "<span class='pagePassive'><a href='index.php?SO=56" . $sayfalamaKosulu . "&SF=" . $sf_indexValue . "'>" . $sf_indexValue . "</a></span>";
                                                }
                                            }
                                        }
                                        if($sf_value != $sf_totalPageCount){
                                            $sf_increasedValue = $sf_value + 1;
                                            echo "<span class='pagePassive'><a href='index.php?SO=56" . $sayfalamaKosulu . "&SF=" . $sf_increasedValue . "'>></a></span>";
                                            
                                            echo "<span class='pagePassive'><a href='index.php?SO=56" . $sayfalamaKosulu . "&SF=" . $sf_totalPageCount . "'>>></a></span>";
                                        }
                                    }
                                ?>
                            </div>
